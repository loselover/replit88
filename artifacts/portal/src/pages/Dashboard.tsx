...
function SetupKeyBanner() {
  const [info, setInfo] = useState<{ key: string; autoGenerated: boolean } | null>(null);
  const [visible, setVisible] = useState(true);

  useEffect(() => {
    fetch(`${BASE_URL}/api/setup-key`)
      .then((r) => r.json())
      .then((data) => {
        if (data.autoGenerated) setInfo(data);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="mb-8 rounded-lg border border-yellow-500/50 bg-yellow-500/10 p-5">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <p className="text-yellow-400 font-semibold mb-1">⚠ 未设置 PROXY_API_KEY — 已自动生成临时密钥</p>
          <p className="text-sm text-yellow-300/80 mb-3">
            此密钥仅在本次运行期间有效。重启后将重新生成。建议在 Replit Secrets 中设置固定的
            <code className="bg-yellow-500/20 px-1 rounded mx-1">PROXY_API_KEY</code>。
          </p>
          <div className="flex items-center gap-2">
            <code className="bg-yellow-500/20 px-3 py-1.5 rounded font-mono text-sm text-yellow-200 flex-1 break-all">
              {info.key}
            </code>
            <CopyButton text={info.key} />
          </div>
        </div>
        <button
          onClick={() => setVisible(false)}
          className="text-yellow-400/60 hover:text-yellow-400 text-lg leading-none flex-shrink-0"
        >
          ✕
        </button>
      </div>
